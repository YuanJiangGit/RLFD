...
class Model(RobertaForSequenceClassification):
    def __init__(self, TEtransformer, SETransformer, config, tokenizer):
    ...
    def get_embedding(self, tokens_features):
        ...
        emb = self.TEtransformer.roberta(input_ids, attention_mask=attn_mask)[0]
        return emb

    def proglines_encode(self, tokens_features):
        ...
        row_mask = torch.tensor(row_mask).cuda()
    ...
    def get_row_rep(self, tokens_features):
        ...
        rows_reps = [torch.zeros(0, 768).cuda()] + rows_reps
        rows_reps = [rows_reps[row_map[i]] for i in range(ori_batch)]

        cls_reps = [torch.zeros(0, 768).cuda()] + cls_reps
        cls_reps = [cls_reps[row_map[i]] for i in range(ori_batch)]
        return rows_reps, cls_reps

    def forward(self, tokens_features, labels_c, labels_f):
        rows_reps, cls_reps = self.get_row_rep(tokens_features[0])
        for i in range(1, len(tokens_features)):
            rows_rep, cls_rep = self.get_row_rep(tokens_features[i])
            if rows_rep is None:
        ...
        labels_f = labels_f[labels_c.bool()][:, 1:prob_f.size(1) + 1]
        return prob_f, labels_f
